...
async update(id: number, dto: UpdateTareaDto) {
  ...
  const flujo: Record<EstadoTarea, EstadoTarea[]> = {
    [EstadoTarea.por_asignar]: [EstadoTarea.pendiente],
    [EstadoTarea.pendiente]: [EstadoTarea.en_curso],
    [EstadoTarea.en_curso]: [EstadoTarea.terminado, EstadoTarea.cancelado],
    [EstadoTarea.terminado]: [],
    [EstadoTarea.cancelado]: [],
  };

  if (dto.estado !== tarea.estado) {
    const permitido = flujo[tarea.estado].includes(dto.estado);

    if (!permitido) {
      throw new BadRequestException(
        `No se puede cambiar de ${tarea.estado} a ${dto.estado}`,
      );
    }
